...
> **BOZZA PER LA CONSULTAZIONE - 2026-06-17 - in attesa di revisione del co-autore.** La versione canonica e autoritativa e il Google Doc. Questo snapshot Markdown e versionato nel repository per citazione e riproducibilita. Canonico IT: https://docs.google.com/document/d/1Mlu56eJNZKmC0LyWH-5FIIAPfKtEDn3Cspo9CoVgWUM
...
5\. Progettare il template in modo forward-compatible con il "single reporting entry point" del Digital Omnibus e con la timeline scaglionata NIS2, così che un'unica presentazione strutturata possa servire più regimi.
...
Proposta (California): valutare di accompagnare il template con un livello di trasparenza/statistica aggregata e anonimizzata delle notifiche a fini di trasparenza e ricerca, sfruttando la stessa tassonomia introdotta dal template, che per la prima volta rende le notifiche aggregabili in modo comparabile.
...
\* Precedente nazionale: il Garante italiano dispone di un canale telematico dal 2021 (docweb 9667201); l'API ne sarebbe la naturale evoluzione.
...
Il Digital Omnibus propone un single entry point operato da ENISA ('report once, share many') che instrada le notifiche di incidente e di violazione tra NIS2, GDPR, DORA, eIDAS e CER: il soggetto notifica una volta, e il back-end instrada la presentazione alle autorità competenti. Se adottato (voto in plenaria del Parlamento europeo atteso il 7-10 luglio 2026), il single entry point diventerebbe disponibile circa 18 mesi dopo l'entrata in vigore delle nuove misure (estendibili a 24 mesi se la Commissione rileva che il portale non soddisfa ancora i requisiti di integrità, affidabilità e riservatezza). Il template di violazione EDPB dovrebbe essere progettato fin dall'inizio per essere compatibile con quel modello dati di punto unico, così che un'unica presentazione strutturata possa soddisfare più regimi. Ciò trasforma il template da silo GDPR a nodo GDPR di un tessuto di reporting pan-regolatorio \- allineandosi all'agenda di semplificazione della stessa Commissione.

Inquadramento importante: il Digital Omnibus è, al momento della stesura, una proposta legislativa ancora in corso di adozione (voto in plenaria del Parlamento europeo atteso il 7-10 luglio 2026; pubblicazione in Gazzetta Ufficiale prevista prima del 2 agosto 2026). Non è ancora norma vigente. Questa raccomandazione è dunque formulata in senso strettamente di forward-compatibility / anticipazione di una proposta in corso, non come conformità a un obbligo esistente.
...
La proposta non è teorica. In allegato a questo paper, come contributo tecnico non vincolante rilasciato in CC BY 4.0, vi è una prima implementazione di riferimento:
...
\* Una fase di pre-valutazione guidata (Parte I, Sezione 3\) e una mappatura ISO/IEC 27035 della tassonomia degli incidenti (Parte III.6).

Metodo: estrazione a livello di cella dalla tabella nativa del file .docx ufficiale, con due parsing indipendenti e riconciliazione tramite diff (zero ambiguità di confine cella). La specifica OpenAPI 3.1, con i securitySchemes di IV.4, è in fase di completamento e sarà rilasciata alle stesse condizioni. \[Riferimento al repository pubblico da inserire prima della presentazione.\]
...
2\. Ancorare il campo 90 alla ENISA SEV (SE \= DPC x EI \+ CB; soglia di rilevanza SE \>= 2\) \+ mappatura indicativa dei livelli; lasciare che il valore calcolato pre-compili il template.  
...
14\. Valutare un livello di trasparenza/statistica aggregata e anonimizzata sul modello California, sfruttando la nuova tassonomia comune.  
...
17\. \[prospettico \- proposta non ancora adottata\] Progettare il template in modo forward-compatible con il single reporting entry point del Digital Omnibus (un'unica presentazione strutturata che serve NIS2/GDPR/DORA/eIDAS/CER). NB: il Digital Omnibus è una proposta legislativa in corso di adozione (voto in plenaria del PE a luglio 2026), qui inquadrata come anticipazione, non come norma vigente.  
...
19\. Consentire il calcolo automatico della severità da input ENISA strutturati (DPC/EI/CB) per la comparabilità tra Autorità e le analisi EDPB.  
...
L'intento degli autori è esplicito: questi artefatti sono offerti per la libera adozione da parte dell'EDPB, delle Autorità nazionali di controllo e di qualunque istituzione UE, in quanto istituzioni pubbliche al servizio dell'interesse pubblico, senza restrizioni oltre l'attribuzione. Gli autori chiedono unicamente di essere citati come fonte \- Nicola Franchetto e Martim Taborda Barata \- e non impongono alcun'altra condizione, costo o barriera. Le Autorità sono libere di adottare, adattare, implementare e ridistribuire lo schema, le tabelle di mappatura e l'implementazione di riferimento, anche nei propri portali.

Nota interna (da rimuovere prima della presentazione): bozza per la validazione di Nicola. Cognome del co-autore (Martim Taborda Barata) confermato. Soglia ENISA SE \< 2 \= Low verificata dalla research sul PDF ufficiale ENISA. Link al repository pubblico da inserire al via di Nicola sulle quattro decisioni di repository. Conflict check: contributo tecnico neutro su uno strumento generale, nessun caso o riferimento client-specific \- rischio basso.

\--gws\_boundary\_68685c05f9d81d12--  
